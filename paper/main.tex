...
\fancyhf{}
\rhead{\thepage}
\lhead{DeepTCR Predicts Therapy Response in rrLBCL}
...
% Title
\title{\textbf{Deep Learning Analysis of T-Cell Receptor Repertoires Predicts Response to CD19 CAR T-cell Therapy in Relapsed/Refractory B-Cell Lymphoma}}
...
\begin{abstract}
\noindent\textbf{Background:} CD19-directed chimeric antigen receptor (CAR) T-cell therapy has improved outcomes for relapsed/refractory (rr) B-cell lymphomas, yet durable responses are not universal. T-cell receptor (TCR) repertoires encode information about immune recognition and T-cell state, but traditional repertoire summary metrics have shown limited predictive value. Deep learning approaches offer the potential to extract complex patterns from high-dimensional TCR data for clinical prediction.

\noindent\textbf{Methods:} We analyzed 239,637 TRB CDR3 amino acid sequences (with V and J gene usage) from 34 patients with rr B-cell lymphoma (18 responders, 16 non-responders). Using DeepTCR, an attention-based multiple instance learning framework, we performed 100-fold Monte Carlo cross-validation to predict therapy response at the patient level and identify predictive clonotypes via attention. In addition, we implemented an unsupervised analysis (no response labels used for representation learning) to learn sequence and patient embeddings and visualize repertoire structure in a two-dimensional UMAP space.

\noindent\textbf{Results:} Our deep learning model achieved robust predictive performance with a mean area under the receiver operating characteristic curve (AUC) of $0.754 \pm 0.035$ (SD) across 100 Monte Carlo cross-validation folds (range: 0.658--0.794, median: 0.765, 95\% CI: 0.747--0.761 from 1,000 bootstrap iterations). The attention mechanism identified 2,379 high-attention sequences (top 1\%), with 72\% of the top 100 predictive sequences originating from responder patients. Enrichment analysis revealed preferential usage of TRBV5-8 (1.54-fold) and TRBJ2-2 (1.17-fold) in high-attention sequences. Unsupervised embeddings provided a complementary qualitative view of repertoire structure and enabled visualization of high-importance sequences in a shared sequence space.

\noindent\textbf{Conclusions:} TCR repertoire analysis using attention-based deep learning provides clinically meaningful prediction of response in rr B-cell lymphoma. The combination of predictive performance, interpretability through attention weights, and complementary unsupervised analyses supports the potential for clinical translation as a pre-therapy biomarker.

\vspace{0.5em}
\noindent\textbf{Keywords:} T-cell receptor, deep learning, CAR T-cell therapy, relapsed/refractory B-cell lymphoma, multiple instance learning, attention mechanism, unsupervised learning
\end{abstract}
...
\subsection{CAR T-Cell Therapy and Predictive Biomarkers}

CD19-directed CAR T-cell therapies have changed the treatment landscape for relapsed/refractory large B-cell lymphoma (rrLBCL), but a substantial fraction of patients fail to achieve durable remission \citep{li2023singlecell_cart19}. This heterogeneity underscores the need for pre-therapy predictive biomarkers to guide treatment selection, optimize manufacturing strategies, and identify patients who may benefit from alternative approaches.

Existing predictors of CAR T outcomes include clinical and tumor-associated factors (e.g., disease burden and laboratory markers), but they do not directly measure the antigen-experienced T-cell repertoire that ultimately mediates tumor control. Repertoire-based modeling provides an opportunity to interrogate the immune response directly from sequence data.

Recent single-cell profiling studies of CD19 CAR T products and patient samples have highlighted associations between T-cell states (memory/exhaustion/regulatory programs) and response, motivating complementary approaches that leverage immune receptor sequence information at scale \citep{li2023singlecell_cart19}.

In addition to supervised prediction, unsupervised representation learning provides an orthogonal way to explore repertoire structure without using outcome labels. In the DeepTCR literature, unsupervised embeddings (e.g., VAE-based) are used to visualize shared ``sequence concepts'' and to contextualize which regions of sequence space are enriched for predictive clonotypes \citep{sidhom2022predictive_concepts}. In this work, we implement an unsupervised analysis adapted to the available inputs (CDR3 + V/J, no HLA) and report both the visualization and patient-level clustering results alongside the supervised MIL model.
...
The primary objective of this study was to develop and rigorously validate a deep learning model for predicting response to CD19 CAR T-cell therapy from pre-therapy TCR repertoires in rr B-cell lymphoma. We hypothesized that attention-based multiple instance learning could identify predictive TCR signatures while maintaining biological interpretability through learned attention weights.
...
This retrospective cohort study analyzed TCR repertoire data from 34 patients with relapsed/refractory B-cell lymphoma treated with CD19-directed CAR T-cell therapy. Patients were classified as responders (n=18) or non-responders (n=16) based on clinical response assessment, using the binary response labels provided with the dataset.
...
\subsection{Unsupervised Representation Learning and Visualization}

To complement supervised multiple instance learning, we implemented unsupervised analyses inspired by Sidhom et al. \citep{sidhom2022predictive_concepts} to visualize structure in TCR sequence space without using response labels during representation learning. Because HLA genotype was not available in our cohort, unsupervised featurization was restricted to TRB CDR3 amino acid sequence and V/J gene usage. We constructed a sparse k-mer representation of CDR3 sequences (k=3) concatenated with one-hot encoded V/J features, projected sequences to a 128-dimensional latent space using truncated singular value decomposition, and visualized the latent space in two dimensions using UMAP. In addition, we constructed patient-level embeddings by mean-pooling sequence embeddings within each patient and performed unsupervised patient clustering (k-means, k=2). All unsupervised outputs and figures are provided in the repository (Scripts 14--16).

\textbf{Implementation details:} Sequence embeddings were constructed from (i) a count-vectorized CDR3 k-mer feature space (k=3; min\_df=2) and (ii) one-hot encoded V and J gene usage, concatenated into a sparse feature matrix. Dimensionality reduction to a 128-dimensional latent space was performed using truncated SVD. UMAP (default parameters) was used to obtain a two-dimensional layout for visualization. To obtain patient-level representations, sequence embeddings were mean-pooled within patient. For visualization scalability, a maximum of 50,000 sequences were plotted, while always retaining the top 1\% highest-attention sequences (when attention weights were available) to preserve the ``predictive'' tail for overlay analysis.

\textbf{Interpretation caveat:} Unsupervised learning can reveal clusters/structure, but it cannot assign semantic labels such as ``responder'' vs ``non-responder'' without using labeled outcomes. Therefore, unsupervised results are reported as descriptive structure and post-hoc agreement with response labels (when computed) and are not treated as an outcome predictor on their own.
...
The study cohort comprised 34 patients with relapsed/refractory B-cell lymphoma, with 18 responders and 16 non-responders to CD19-directed CAR T-cell therapy. TCR sequencing yielded 239,637 productive TRB sequences across all patients (Figure~\ref{fig:cohort}).
...
\subsection{Unsupervised Repertoire Structure}

We next performed unsupervised representation learning and visualization to obtain a label-agnostic view of repertoire structure. Patient-level clustering using mean-pooled unsupervised embeddings yielded an imbalanced partition (one dominant cluster and a single outlier patient; Figure~\ref{fig:s10}), and post-hoc agreement with response labels was near chance (ARI $\approx$ 0; see \texttt{results/unsupervised/cluster\_report.txt}). This indicates that in this cohort, the unsupervised patient embedding alone is insufficient to separate responders from non-responders.

At the sequence level, a two-dimensional UMAP visualization of the unsupervised latent space revealed multimodal structure consistent with multiple sequence ``concepts'' across the cohort (Figure~\ref{fig:s11}). Overlaying high-attention sequences (proxy for supervised importance) highlighted enrichment of model-important clonotypes in localized regions of sequence space, providing an interpretable bridge between the supervised MIL model and unsupervised repertoire geometry.

\textbf{Unsupervised validation:} We quantified internal cluster quality and stability across a range of cluster counts (k=2--6) using standard unsupervised validation metrics (silhouette, Davies--Bouldin, Calinski--Harabasz) and assessed stability under repeated runs and patient subsampling using adjusted Rand index (ARI) agreement. Validation results are summarized in Figure~\ref{fig:s15} and in \texttt{results/unsupervised/unsupervised\_validation\_report.txt}.
...
This study demonstrates that attention-based deep learning analysis of pre-therapy TCR repertoires can predict response in relapsed/refractory B-cell lymphoma with clinically meaningful accuracy. The achieved AUC of $0.754 \pm 0.035$ (SD) across 100 Monte Carlo folds, with a bootstrap-derived 95\% confidence interval of 0.747--0.761, represents robust predictive performance within this cohort.
...
\subsection{Comparison with Existing Predictors in CAR T-cell Therapy}

Clinical outcome prediction in CAR T-cell therapy has used a range of clinical, tumor, and product-associated factors, but robust pre-therapy biomarkers remain an active area of research. Sequence-based repertoire modeling offers a complementary signal source by directly representing the adaptive immune repertoire, and can be integrated with other modalities in future work.
...
The success of attention-based MIL for repertoire classification supports the hypothesis that predictive information is concentrated in a subset of TCR sequences within each patient's repertoire. This aligns with the biological model wherein antigen-experienced clonotypes represent a small fraction of the total T-cell pool, but can drive therapy outcomes.

V-gene usage patterns identified by attention analysis may reflect germline-encoded biases in antigen recognition. Enrichment of specific V/J genes among high-attention sequences suggests shared sequence concepts that may relate to common antigenic exposures relevant to CAR T-cell therapy outcomes.
...
We have demonstrated that attention-based deep learning analysis of TCR repertoires provides clinically meaningful prediction of response in relapsed/refractory B-cell lymphoma. The DeepTCR model achieved an AUC of $0.754 \pm 0.035$ (SD) across 100 Monte Carlo cross-validation folds (95\% CI: 0.747--0.761 from 1,000 bootstrap iterations), with attention weights identifying 2,379 high-attention sequences associated with response. Notably, 72\% of the top 100 predictive sequences originated from responder patients, and enrichment analysis revealed preferential usage of TRBV5-8 (1.54-fold) in high-attention sequences.
...
\subsection*{Figure S10: Unsupervised Patient Clustering}
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figureS10_unsupervised_patient_clusters.png}
\caption{\textbf{Unsupervised Patient Clustering.} Patient-level embeddings were constructed by unsupervised featurization of TRB CDR3 sequences and V/J gene usage followed by mean pooling within each patient. Clustering (k=2) was performed without using response labels and visualized in two dimensions using PCA. This analysis provides an unsupervised view of patient-level repertoire similarity.}
\label{fig:s10}
\end{figure}

\subsection*{Figure S11: Unsupervised Sequence-Space Visualization}
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figureS11_unsupervised_sequence_space.png}
\caption{\textbf{Unsupervised Sequence-Space Visualization.} A two-dimensional visualization of an unsupervised latent embedding of TRB sequences derived from CDR3 k-mer features and V/J gene usage. Points are colored by response label (Responder = green, Non-responder = orange) to aid visual interpretation, and an overlay highlights the top decile of sequences by attention-based importance (proxy for supervised MIL importance).}
\label{fig:s11}
\end{figure}

\subsection*{Figure S12: Featurization Schematics}
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figureS12_featurization_schematics.png}
\caption{\textbf{Featurization Schematics for Unsupervised and Supervised Models.} Conceptual overview of (A) unsupervised featurization and UMAP visualization used in this study (no labels used for representation learning) and (B) supervised DeepTCR multiple instance learning with CNN featurization and attention-based pooling for patient-level prediction.}
\label{fig:s12}
\end{figure}

\subsection*{Figure S13: Unsupervised Pipeline Overview}
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figureS13_unsupervised_pipeline.png}
\caption{\textbf{Unsupervised Pipeline Overview.} High-level schematic of the unsupervised workflow implemented in this study: TRB CDR3 and V/J gene usage are featurized, embedded into a latent space, and visualized in two dimensions using UMAP. A patient-level representation is obtained by pooling sequence embeddings within each patient, enabling unsupervised clustering.}
\label{fig:s13}
\end{figure}

\subsection*{Figure S14: Unsupervised Learning Pipeline (Block Diagram)}
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figureS14_unsupervised_pipeline_blocks.png}
\caption{\textbf{Unsupervised Learning Pipeline (Block Diagram).} A simplified end-to-end view of the unsupervised workflow, analogous to the supervised pipeline figure: sequence inputs are filtered, featurized, embedded into a latent space, visualized with UMAP, and pooled to the patient level for clustering.}
\label{fig:s14}
\end{figure}

\subsection*{Figure S15: Unsupervised Validation (Quality and Stability)}
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figureS15_unsupervised_validation.png}
\caption{\textbf{Unsupervised Validation (Quality and Stability).} Internal validity metrics (silhouette, Davies--Bouldin) and stability (ARI agreement across repeated runs and patient subsampling) for patient-level clustering across a range of cluster counts (k).}
\label{fig:s15}
\end{figure}
