...
\subsection{Unsupervised Learning: VAE-Inspired Clustering}

In addition to supervised prediction, we performed unsupervised analysis to discover natural patterns in TCR repertoires without using response labels. This approach is inspired by Variational Autoencoder (VAE) methodologies that have proven effective for biological sequence analysis \citep{kingma2014vae,lopez2018deep}.

\textbf{Feature Extraction:} TCR sequences were represented using k-mer frequency features (k=3), capturing local amino acid patterns within CDR3 regions. This representation yielded 6,213 unique 3-mer features across the dataset. V-gene and J-gene identities were encoded as one-hot vectors (49 V-genes, 13 J-genes), producing a combined feature matrix of 6,275 dimensions per sequence.

\textbf{Dimensionality Reduction:} Principal Component Analysis (PCA) was applied to reduce the high-dimensional feature space to a 50-dimensional latent representation, analogous to the latent space in VAE models. This latent space captures the primary modes of variation in TCR sequence composition and gene usage patterns.

\textbf{Clustering:} K-means clustering was applied to the latent space representations to identify natural groupings of TCR sequences. The number of clusters (k=5) was selected based on interpretability and cluster size distribution. Cluster assignments were analyzed for enrichment of responder versus non-responder sequences.

\textbf{Visualization:} t-distributed Stochastic Neighbor Embedding (t-SNE) was applied to visualize the high-dimensional latent space in two dimensions, enabling assessment of cluster separation and response status distribution.
...
The unsupervised VAE-inspired analysis revealed distinct clustering patterns in the TCR repertoire that partially segregate responders from non-responders (Figure~\ref{fig:s10}). Key findings include:

\begin{itemize}
    \item \textbf{Cluster identification:} Five distinct TCR clusters were identified based on k-mer sequence features and gene usage patterns
    \item \textbf{Responder-enriched cluster:} Cluster 4 showed the highest responder enrichment (1.82-fold), containing 99.3\% responder-derived sequences
    \item \textbf{Cluster 2 enrichment:} This cluster showed moderate responder enrichment (1.05-fold, 57.5\% responder sequences)
    \item \textbf{Dominant cluster:} Cluster 3 contained the majority of sequences (52.9\%) with near-equal distribution between response groups
\end{itemize}

\textbf{Patient-level analysis} revealed that responders showed higher representation in Cluster 2 (21.9\% vs. 19.4\% in non-responders) and exclusive representation in Cluster 4 (0.3\% vs. 0.0\%). These differences, while modest at the overall level, suggest that specific TCR subpopulations may be associated with CAR-T product fitness.

\textbf{Gene usage patterns by cluster} revealed distinct V-J preferences:
\begin{itemize}
    \item Cluster 0: Enriched for TRBV20-1, TRBV7-9, TRBV9 with TRBJ2-1, TRBJ2-7
    \item Cluster 2: Enriched for TRBV12-4, TRBV4-1 with TRBJ1-1, TRBJ2-5
    \item Cluster 4 (responder-enriched): Dominated by TRBV3-1 with TRBJ1-3
\end{itemize}

The convergence of supervised and unsupervised findings---both identifying TRBV3-1 as a distinguishing feature---provides independent validation of this gene's association with CAR-T response.
...
\caption{\textbf{Unsupervised Clustering Analysis.} (A) t-SNE visualization of TCR repertoire colored by cluster assignment (k=5 clusters identified). (B) t-SNE visualization colored by response status (green: responder, red: non-responder). The unsupervised analysis reveals natural groupings in TCR repertoire that partially segregate by response status. Cluster 4 (0.3\% of sequences) shows 1.82-fold responder enrichment with 99.3\% of sequences from responder patients, suggesting this TCR subpopulation may mark T cells with favorable properties for CAR-T efficacy.}
\label{fig:s10}
\end{figure}
