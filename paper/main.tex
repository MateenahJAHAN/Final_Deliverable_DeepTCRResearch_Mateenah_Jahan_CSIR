...
\fancyhf{}
\rhead{\thepage}
\lhead{DeepTCR Predicts CAR-T Cell Therapy Response}
...
% Title
\title{\textbf{Deep Learning Analysis of T-Cell Receptor Repertoires Predicts CAR-T Cell Therapy Response in Relapsed/Refractory Large B-Cell Lymphoma}}
...
\begin{abstract}
\noindent\textbf{Background:} Chimeric antigen receptor T-cell (CAR-T) therapy has revolutionized treatment of relapsed/refractory large B-cell lymphoma (rrLBCL), yet durable response rates remain variable. The T-cell receptor (TCR) repertoires of CAR-T infusion products encode information about T-cell functional states, but traditional repertoire metrics have shown limited predictive value. Deep learning approaches offer potential to extract complex patterns from high-dimensional TCR data for clinical prediction.

\noindent\textbf{Methods:} We analyzed 239,637 TCR-beta sequences from CAR-T cell infusion products of 34 patients with relapsed/refractory large B-cell lymphoma treated with axicabtagene ciloleucel (Axi-cel) (18 responders, 16 non-responders) \citep{li2023singlecell}. Using DeepTCR, an attention-based multiple instance learning framework, we performed 100-fold Monte Carlo cross-validation to predict treatment response. The model integrates CDR3 amino acid sequences with V-gene and J-gene usage patterns, employing an attention mechanism to identify predictive TCR clonotypes. Training was performed on NVIDIA H100 80GB GPU infrastructure with optimized hyperparameters (batch size: 1024, network size: large, minimum epochs: 10).
...
\noindent\textbf{Conclusions:} TCR repertoire analysis using attention-based deep learning provides clinically meaningful prediction of CAR-T cell therapy response in large B-cell lymphoma. The model's combination of high predictive performance, biological interpretability through attention weights, and computational efficiency suggests potential for clinical translation in optimizing CAR-T manufacturing and patient selection.

\vspace{0.5em}
\noindent\textbf{Keywords:} T-cell receptor, deep learning, CAR-T cell therapy, CD19, large B-cell lymphoma, multiple instance learning, attention mechanism
\end{abstract}
...
Chimeric antigen receptor T-cell (CAR-T) therapy has fundamentally transformed the treatment landscape for relapsed/refractory large B-cell lymphoma (rrLBCL) \citep{li2023singlecell}. CD19-directed CAR-T cell products such as axicabtagene ciloleucel (Axi-cel) have demonstrated remarkable clinical responses, with complete response rates of approximately 40--50\% in patients who have failed multiple prior therapies. However, response rates remain highly variable, with the majority of patients not achieving durable remissions \citep{sharma2017primary}. This heterogeneity underscores the critical need for predictive biomarkers to guide treatment selection and patient stratification.

Current predictive biomarkers for CAR-T cell therapy response include tumor burden, inflammatory markers (such as CRP, ferritin, and LDH), and T-cell phenotypes in the infusion product. While these markers show association with response in specific contexts, their predictive accuracy remains suboptimal. Tumor burden metrics require imaging and can change rapidly during bridging therapy. T-cell phenotypes in the infusion product, including memory, exhausted, and regulatory T-cell states, have been associated with outcomes but require detailed immunophenotyping or single-cell analysis \citep{li2023singlecell}.

Large B-cell lymphoma represents an important model system for CAR-T biomarker discovery. rrLBCL patients treated with Axi-cel show heterogeneous outcomes that are influenced by both tumor-intrinsic factors and the functional characteristics of the CAR-T infusion product. Understanding the T-cell receptor repertoire of these products may reveal features predictive of clinical response.
...
The primary objective of this study was to develop and rigorously validate a deep learning model for predicting CAR-T cell therapy response from TCR repertoires in CAR-T infusion products for large B-cell lymphoma patients. We hypothesized that attention-based multiple instance learning could identify predictive TCR signatures while maintaining biological interpretability through learned attention weights.

Secondary objectives included: (1) quantifying model robustness through extensive Monte Carlo cross-validation, (2) characterizing TCR repertoire features associated with CAR-T therapy response, and (3) evaluating the computational efficiency of deep learning approaches for clinical translation.
...
This retrospective cohort study analyzed TCR repertoire data from CAR-T cell infusion products of 34 patients with relapsed/refractory large B-cell lymphoma (rrLBCL) treated with axicabtagene ciloleucel (Axi-cel) at MD Anderson Cancer Center \citep{li2023singlecell}. The data were derived from a larger single-cell RNA sequencing atlas of 59 patients, with 34 patients included based on evaluability criteria and absence of additional trial therapies that might confound outcomes.

Patients were classified as responders (n=18) or non-responders (n=16) based on PET/CT assessment at 3-month follow-up, an important landmark for long-term outcomes. Responders achieved complete response (CR), while non-responders included patients with stable disease (SD), partial response (PR), or progressive disease (PD).

TCR sequences were extracted from single-cell RNA sequencing of CAR-T infusion products collected prior to infusion. The near-balanced cohort design (18 responders, 16 non-responders) helps prevent severe class imbalance from confounding model training. Patient and repertoire characteristics are summarized in Table~\ref{tab:patient_characteristics}.
...
TCR repertoire data were derived from single-cell RNA sequencing (scRNA-seq) of CAR-T cell infusion products using 10x Genomics Chromium platform with 5' immune profiling \citep{li2023singlecell}. This approach enables simultaneous capture of gene expression and paired TCR alpha/beta chain sequences from individual cells. The original dataset contained 417,167 high-quality single cells from 59 patients; we utilized the TCR-beta sequences from 34 evaluable patients (mean: 7,048 sequences per patient).
...
\begin{enumerate}
    \item \textbf{Sequence alignment and annotation:} TCR sequences were aligned to the human TRB reference genome (IMGT nomenclature) to identify V genes, J genes, and CDR3 boundaries.
...
The study cohort comprised 34 patients with relapsed/refractory large B-cell lymphoma, with 18 responders and 16 non-responders to CAR-T cell therapy (axicabtagene ciloleucel). TCR repertoire analysis of CAR-T infusion products yielded 239,637 productive TRB sequences across all patients (Figure~\ref{fig:cohort}).
...
This study demonstrates that attention-based deep learning analysis of CAR-T cell infusion product TCR repertoires can predict therapy response in large B-cell lymphoma with clinically meaningful accuracy. The achieved AUC of $0.754 \pm 0.035$ (SD) across 100 Monte Carlo folds, with a bootstrap-derived 95\% confidence interval of 0.747--0.761, represents robust predictive performance that compares favorably with existing biomarkers for CAR-T therapy outcomes.
...
Second, the attention mechanism provides biological interpretability that distinguishes this approach from traditional ``black box'' machine learning. By identifying specific TCR sequences that drive predictions, the model generates testable hypotheses about functionally relevant T-cell populations within the CAR-T product. Our analysis identified 2,379 high-attention sequences, with 72\% of the top 100 originating from responder patients. These high-attention sequences may represent clonotypes with favorable functional characteristics (e.g., memory phenotypes, reduced exhaustion), offering candidates for functional validation studies.
...
Current predictive biomarkers for CAR-T cell therapy response show variable performance. Tumor burden metrics, inflammatory markers (CRP, ferritin, LDH), and T-cell phenotypes in infusion products have been associated with outcomes but demonstrate limited predictive accuracy individually. Prior studies using flow cytometry-based T-cell phenotyping have shown AUCs typically ranging from 0.60 to 0.70 \citep{li2023singlecell}.

Our TCR-based approach offers several potential advantages. Analysis of the infusion product is non-invasive and can be performed prior to manufacturing completion for quality control purposes. The model directly interrogates the T-cell repertoire diversity and composition that may reflect functional fitness. Furthermore, attention weights provide mechanistic insight not available from aggregate biomarkers, potentially identifying specific T-cell clones associated with durable responses.

\subsection{Biological Implications}

The success of attention-based MIL for repertoire classification supports the hypothesis that predictive information is concentrated in a subset of TCR sequences within each patient's CAR-T infusion product. This aligns with the biological model wherein CAR-T cell products contain heterogeneous T-cell populations, with specific subsets (e.g., memory T cells, less exhausted phenotypes) driving clinical responses \citep{li2023singlecell}.

V-gene usage patterns identified by attention analysis may reflect germline-encoded biases in T-cell development and function. Certain V genes may be associated with T-cell phenotypes that are more favorable for CAR-T expansion and persistence in vivo. The enrichment of specific V-gene families in responder repertoires suggests shared T-cell developmental or functional characteristics associated with durable responses.

\subsection{Limitations}

Several limitations of this study warrant consideration. First, the sample size of 34 patients, while sufficient for proof-of-principle, limits statistical power and generalizability. Validation in larger, independent cohorts across different CAR-T products and institutions is essential before clinical implementation.

Second, the cohort design (18 responders, 16 non-responders) may not reflect real-world response rates, which vary across CAR-T products and patient populations. Model calibration should be evaluated in populations with varying class distributions.

Third, our analysis used TCR sequences from the infusion product, which may undergo significant expansion and contraction dynamics in vivo. Post-infusion TCR repertoire analysis at various timepoints may provide additional predictive information about durable response.
...
Several extensions of this work are warranted. Multi-center validation studies with larger sample sizes across different CAR-T products (Axi-cel, tisagenlecleucel, lisocabtagene maraleucel) would establish generalizability across manufacturing processes and patient populations. Integration of TCR features with existing biomarkers (inflammatory markers, T-cell phenotypes, tumor burden) may improve predictive accuracy through complementary information. Longitudinal sampling post-infusion could enable dynamic response prediction and early detection of resistance or relapse.

Functional validation of high-attention TCR sequences through single-cell multi-omics or T-cell functional assays would clarify whether these sequences represent specific T-cell phenotypes (memory, stem-like, exhausted). Identification of shared TCR motifs across responders could inform CAR-T manufacturing optimization or patient selection strategies.

%==============================================================================
\section{Conclusion}
%==============================================================================

We have demonstrated that attention-based deep learning analysis of TCR repertoires from CAR-T cell infusion products provides clinically meaningful prediction of therapy response in large B-cell lymphoma. The DeepTCR model achieved an AUC of $0.754 \pm 0.035$ (SD) across 100 Monte Carlo cross-validation folds (95\% CI: 0.747--0.761 from 1,000 bootstrap iterations), with attention weights identifying 2,379 high-attention sequences associated with treatment response. Notably, 72\% of the top 100 predictive sequences originated from responder patients, and enrichment analysis revealed preferential usage of TRBV5-8 (1.54-fold) in high-attention sequences.

This approach combines three desirable properties for clinical translation: (1) predictive accuracy comparable to or exceeding existing biomarkers for CAR-T therapy, validated through rigorous bootstrap analysis, (2) biological interpretability through attention-based sequence identification enabling hypothesis generation for functional studies of CAR-T product composition, and (3) computational efficiency enabling real-time prediction on modern GPU hardware. These findings support further development of TCR-based biomarkers for optimizing CAR-T cell therapy, with validation in larger cohorts as the critical next step toward clinical implementation.
...
The original single-cell RNA sequencing data from CAR-T cell infusion products is available from the Gene Expression Omnibus (GEO) under accession number \textbf{GSE241783} \citep{li2023singlecell}. The DeepTCR framework is available at \url{https://github.com/sidhomj/DeepTCR}. The processed TCR dataset used in this study is included in the repository.
